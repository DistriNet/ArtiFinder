...
from artifinder.model import ArtifactBadge, badge
from artifinder.util import text_encoding
...
class SecartifactsArtifact:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert the artifact to a dictionary for JSON serialization."""
        return {
            "title": self.title,
            "artifact_urls": self.artifact_urls,
            "appendix_url": self.appendix_url,
            "paper_url": self.paper_url,
            "badges": [str(badge) for badge in self.badges] if self.badges else None
        }

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "SecartifactsArtifact":
        """Create an artifact from a dictionary."""
        return SecartifactsArtifact(
            title=data.get("title", ""),
            artifact_urls=data.get("artifact_urls", []),
            appendix_url=data.get("appendix_url"),
            paper_url=data.get("paper_url"),
            badges=[badge.ArtifactBadge.from_string(b) for b in data.get("badges", [])] if data.get("badges") else None
        )
    ...
    def _parse_artifacts(self, raw: list[dict[str, Any]]):
        ...
        for entry in raw:
            ...
            if "artifact_url" in entry:
                if type(entry["artifact_url"]) is list: entry["artifact_url"] = entry["artifact_url"][0]
