...
class Ranker:
    ...
    def _prepare_links(self, links: Iterable[LinkType]) -> list[tuple[RankedLink, LinkType]]:
        return [(RankedLink.from_dict({**link.to_dict(), "score": 0, "breakdown": []}), link ) for link in links]
    ...
    def _rank_link(self, phase: RankPhase, root_ctx: RootContext, rlink: RankedLink, link: LinkType) -> None:
        ...
        for rule in phase.rules():
            ...
            try:
                ...
                log.error(f"Error evaluating rule {rule.__class__.__name__} for link {str(ctx.link)}: {e}")
    def rank_link(
