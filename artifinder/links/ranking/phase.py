...
from artifinder.util.cache import LRUCache
...
class RequestPhase(RankPhase):
    ...
    def _request(self, url: urllib3.util.Url) -> requests.Response | None:
        if url.host is None:
            return None
        timeout = (3, 3) if not "web.archive.org" in url.url else (60, 60)
        lock = self._get_lock(url)
        with lock:
            r: requests.Response | None = None
            try:
                r = self._session.get(url.url, stream=True, timeout=timeout)
                if (
                ...
                if not "web.archive.org" in url.url:
                    return self._request(safe_parse_url('https://web.archive.org/web/' + date.today().strftime('%Y_%m_%d').replace('_', '') + 'if_/' + url.url))

            if not "web.archive.org" in url.url and r and (not r.ok or r.content is None):
                return self._request(safe_parse_url('https://web.archive.org/web/' + date.today().strftime('%Y_%m_%d').replace('_', '') + 'if_/' + url.url))
            return r
